...
background 子 agent 使用 `background` 工具作用域（仅基础执行工具）。team 子 agent 使用 `subagent` 作用域：在基础工具之外还可使用 `post_team_message` / `fetch_team_messages`，与同 `group_id` 的队友通信。子 agent 均不能再创建子 agent，也不能管理 worktree 或 team 生命周期。
...
- 依赖：`mcpsdk`（封装官方 `mcp` 客户端），在 `.agent_env` 中通过 `pip install -r requirements.txt` 安装。
- 主 agent 工具：`notion_mcp_list_tools`、`notion_mcp_call_tool`，通过环境变量连接本地 Notion MCP（默认 `npx -y @notionhq/notion-mcp-server`，需配置 `NOTION_TOKEN`）或 SSE 托管端点（见 `.env.example`）。
- Team 子 agent 通过 `group_id` 分组，使用 `post_team_message` / `fetch_team_messages` 通信；主 agent 可用 `peek_team_messages` 查看某组的近期消息。
...
- [x] 文件编辑功能
  ...
  - [x] 集成到主agent工具集
- [x] MCP SDK集成
  - [x] 添加 mcpsdk / mcp 依赖（在虚拟环境 `.agent_env` 中安装）
  - [x] 环境配置支持（`.env.example` + Notion MCP 工具）
...
- [x] 添加消息机制，让同一个组的子agent可以相互交流（`group_id` + `post_team_message` / `fetch_team_messages`）
- [ ] 添加skill机制
  -[ ] 使用gog.skill来实现对日程的管理